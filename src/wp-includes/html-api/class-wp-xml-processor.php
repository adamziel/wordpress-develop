 ...
 *
 * Scans through an XML document to find specific nodes, then
 * transforms those nodes. If they're tags, it can add, remove,
 * or update the values of the XML attributes within that tag (opener).
 * If they're text nodes, it can change their text contents
 *
 ...
 * | Find next image tag.                                      | `$tags->next_tag( array( 'name' => 'wp:image' ) );`                              |
 ...
 *
 *     $total_todos = 0;
 *     while ( $p->next_tag( array( 'name' => 'wp:todo-list' ) ) ) {
 *         $p->set_bookmark( 'list-start' );
 *         while ( $p->next_tag( array( 'closers' => 'visit' ) ) ) {
 *             if ( 'wp:todo' === $p->get_tag() && $p->is_closer() ) {
 ...
 *             }
 *
 *             if ( 'wp:todo-item' === $p->get_tag() && ! $p->is_closer() ) {
 *                 $total_todos++;
 *             }
 *         }
 *     }
...
class WP_XML_Processor {
	 ...
	 *
	 *     @type string|null $name     Which tag to find, or `null` for "any tag."
	...
	public function next_tag( $query = null ) {
		...
		if ( isset( $query['closers'] ) && 'visit' === $query['closers'] ) {
			_doing_it_wrong(
				__METHOD__,
	...
	private function step_in_prolog() {
		...
		switch ( $this->get_token_type() ) {
			case '#text':
				// Skip over text nodes in prolog
	...
	public function matches_breadcrumbs( $breadcrumbs ) {
		...
		for ( $i = count( $this->stack_of_open_elements ) - 1; $i >= 0; $i-- ) {
			$name  = $this->stack_of_open_elements[ $i ];
	...
	private function push_open_element( $name ) {
		array_push(
	...
	protected function base_class_next_token() {
		...
		$ends_at = strpos( $this->xml, '>', $this->bytes_already_parsed );
		if ( false === $ends_at ) {
			$this->parser_state         = self::STATE_INCOMPLETE_INPUT;
			$this->bytes_already_parsed = $was_at;
		...
		if ( $this->is_closing_tag && $ends_at !== $this->bytes_already_parsed ) {
			$this->last_error = self::ERROR_SYNTAX;
			_doing_it_wrong(
				__METHOD__,
		...
		$this->bytes_already_parsed = $ends_at + 1;
		 ...
		 * tag sequence.
		 */
		$name_starts_at = $this->name_starts_at;
		$name_length    = $this->name_length;
		$ends_at        = $this->token_starts_at + $this->token_length;
		...
		$this->token_length    = $this->bytes_already_parsed - $this->token_starts_at;
		$this->text_starts_at  = $ends_at;
		$this->text_length     = $this->name_starts_at - $this->text_starts_at;
		$this->name_starts_at  = $name_starts_at;
	 ...
	 *     $in_list = false;
	 *     while ( $p->next_tag( array( 'closers' => $in_list ? 'visit' : 'skip' ) ) ) {
	 *         if ( 'UL' === $p->get_tag() ) {
	 *             if ( $p->is_closer() ) {
	 *                 $in_list = false;
	 ...
	 *
	 * @param string $name The tag name which will close the PCDATA region.
	 * @return false|int Byte offset of the closing tag, or false if not found.
	 */
	private function skip_pcdata( $name ) {
		$xml        = $this->xml;
		...
		while ( false !== $at && $at < $doc_length ) {
			$at                   = strpos( $this->xml, '</' . $name, $at );
			$this->name_starts_at = $at;
	...
	private function parse_next_tag() {
		...
		while ( false !== $at && $at < $doc_length ) {
			...
			/*
			 * If there's no more tags to parse and we're already past the root
			 * element, then the parsing may either be finished or we may still
			 * be waiting for more input.
			 */
			if ( false === $at ) {
				switch ( $this->parser_context ) {
					case self::IN_MISC_CONTEXT:
						...
						if ( $this->bytes_already_parsed >= $doc_length ) {
							/**
							 * If we're past whitespace after the root element, we can never
							 * be sure whether the parse is complete or are we still waiting
							 * for a comment or a processing directive.
							 */
							$this->parser_state = self::STATE_AMBIGOUS_COMPLETE;
							return false;
						}

						// If we've encountered anything else than whitespace or a < character
						// after the root element, it's a syntax error.
						$this->last_error = self::ERROR_SYNTAX;
						_doing_it_wrong(
							__METHOD__,
							__( 'Invalid syntax encountered.' ),
							'WP_VERSION'
						);
						return true;
					default:
						/*
						 * If we're not finished with the root element yet,
						 * then the document is incomplete.
						 */
						$this->parser_state = self::STATE_INCOMPLETE_INPUT;
						return false;
				}
			 ...
			 */
			$name_length = $this->parse_name( $at + 1 );
			if ( $name_length > 0 ) {
				++$at;
				$this->parser_state         = self::STATE_MATCHED_TAG;
				$this->name_starts_at       = $at;
				$this->name_length          = $name_length;
				$this->bytes_already_parsed = $at + $this->name_length;
	...
	private function after_tag() {
		...
		$this->name_starts_at  = null;
		$this->name_length     = null;
	...
	public function get_tag() {
		if ( null === $this->name_starts_at ) {
			return null;
		}

		$name = substr( $this->xml, $this->name_starts_at, $this->name_length );
	 ...
	 *     $p->next_tag( array( 'name' => 'wp:content', 'closers' => 'visit' ) );
	 *     $p->is_closer() === false;
	 *
	 *     $p->next_tag( array( 'name' => 'wp:content', 'closers' => 'visit' ) );
	 *     $p->is_closer() === true;
	...
	public function set_attribute( $name, $value ) {
		...
		if ( isset( $this->attributes[ $name ] ) ) {
			 ...
			 */
			$this->lexical_updates[ $name ] = new WP_HTML_Text_Replacement(
				$this->name_starts_at + $this->name_length,
				0,
				' ' . $updated_attribute
			);
		}
	 ...
	 *                                     1 for "first" tag, 3 for "third," etc.
	 *                                     Defaults to first tag.
	 *     @type string      $closers  "visit" or "skip": whether to stop on tag closers, e.g. </wp:content>.
	 * }
	 */
	private function parse_query( $query ) {
		...
		if ( isset( $query['closers'] ) ) {
			$this->stop_on_closers = 'visit' === $query['closers'];
		}
	}
	...
	private function matches() {
		...
		if ( null !== $this->sought_name ) {
			...
			for ( $i = 0; $i < $this->name_length; $i++ ) {
				if ( $this->xml[ $this->name_starts_at + $i ] !== $this->sought_name[ $i ] ) {
					return false;
				}
			}
		}
	...
	/**
	 * Parser Ambigous Complete State.
	 *
	 * Indicates that the parser has processed what looks like an entire document,
	 * but it's not clear if it's complete or if it was truncated and we are still
	 ...
	 * @access private
	 */
	const STATE_AMBIGOUS_COMPLETE = 'STATE_AMBIGOUS_COMPLETE';
