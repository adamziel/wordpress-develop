...
class Tests_XmlApi_WpXmlProcessor extends PHPUnit_Framework_TestCase {
	 ...
	 */
	public function test_get_returns_null_before_finding_tags() {
	 ...
	 * @covers WP_XML_Processor::get_tag
	 */
	public function test_get_returns_null_when_not_in_open_tag() {
		$processor = new WP_XML_Processor( '<wp:content>Test</wp:content>' );
	 ...
	 * @covers WP_XML_Processor::get_tag
	 */
	public function test_get_returns_open_name() {
		$processor = new WP_XML_Processor( '<wp:content>Test</wp:content>' );
	...
	public function test_is_empty_element_matches_input_xml( $xml, $flag_is_set ) {
		...
		$processor->next_tag( array( 'closers' => 'visit' ) );
	...
	public function test_get_attribute_returns_null_when_in_closing_tag() {
		...
		$this->assertTrue( $processor->next_tag( 'wp:content' ), 'Querying an existing tag did not return true' );
		$this->assertTrue( $processor->next_tag( array( 'closers' => 'visit' ) ), 'Querying an existing closing tag did not return true' );
	 ...
	 * @covers WP_XML_Processor::get_attribute
	 */
	public function test_attributes_are_rejected_in_closers() {
		$processor = new WP_XML_Processor( '<wp:content>Test</wp:content wp:post-type="test">' );

		$this->assertTrue( $processor->next_tag( 'wp:content' ), 'Querying an existing tag did not return true' );
		$this->assertFalse( $processor->next_tag( array( 'closers' => 'visit' ) ), 'Querying an existing but invalid closing tag did not return false.' );
	}
	...
	public static function data_xml_nth_token_substring() {
		return array(
			...
			// Text.
			'Text'                          => array( 'Just text<elem>', 1, 'Just text' ),
			'Text in DIV'                   => array( '<wp:content>Text<wp:content>', 2, 'Text' ),
			'Text before DIV'               => array( 'Text<wp:content>', 1, 'Text' ),
			'Text after comment'            => array( '<elem><!-- comment -->Text</elem>', 3, 'Text' ),
			'Text before comment'           => array( '<elem>Text<!-- c --> ', 2, 'Text' ),
	 ...
	 *
	 * @covers WP_XML_Processor::next_tag
	 * @covers WP_XML_Processor::is_closer
	 */
	public function test_next_should_stop_on_closers_only_when_requested() {
		$processor = new WP_XML_Processor( '<wp:content><photo /></wp:content>' );

		$this->assertTrue( $processor->next_tag( array( 'name' => 'wp:content' ) ), 'Did not find desired tag opener' );
		$this->assertFalse( $processor->next_tag( array( 'name' => 'wp:content' ) ), 'Visited an unwanted tag, a tag closer' );
		...
		$this->assertFalse( $processor->is_closer(), 'Indicated a tag opener is a tag closer' );
		$this->assertTrue(
			$processor->next_tag(
				array(
					'name'    => 'wp:content',
		...
		$this->assertTrue( $processor->is_closer(), 'Indicated a tag closer is a tag opener' );

		$processor = new WP_XML_Processor( '<wp:content>' );
		$this->assertTrue( $processor->next_tag( array( 'closers' => 'visit' ) ), "Did not find a tag opener when closers was set to 'visit'" );
		$this->assertFalse( $processor->next_tag( array( 'closers' => 'visit' ) ), "Found a closer where there wasn't one" );
	}
	...
	public function test_internal_pointer_returns_to_original_spot_after_inserting_content_before_cursor() {
		...
		$this->assertFalse( $tags->is_closer() );
	}
	 ...
	 * @covers WP_XML_Processor::set_attribute
	 */
	public function test_set_attribute_on_a_non_existing_does_not_change_the_markup() {
		$processor = new WP_XML_Processor( self::XML_SIMPLE );
	 ...
	 */
	public function test_attribute_ops_on_closer_do_not_change_the_markup() {
		...
		$this->assertFalse( $processor->is_closer(), 'Skipped tag opener' );
		...
		$this->assertTrue( $processor->is_closer(), 'Skipped tag closer' );
	 ...
	 */
	public function test_next_returns_false_for_incomplete_syntax_elements( $incomplete_xml ) {
	...
	public function test_single_text_node_with_taglike_text() {
		$processor = new WP_XML_Processor( 'This is a text node< /A>' );
	 ...
	 * @return void
	 */
	public function test_next_by_breadcrumbs() {
		// Initialize the WP_XML_Processor with the given XML string
		$processor = new WP_XML_Processor( '<root><wp:post><content><image /></content></wp:post></root>' );
